...
Ans: An Inner class isa class which is nested within another class. An Inner class has access rights 
...
by definingg any breaking logic in the body of the statement blocks. 
...
Ans: In java, main() method can't return any data and hence, ite's always declared with a void 
return type. 
...
import therm separately if required. 
...
static method,we wil not get an complitaion error,nor the impact of overriding when running the 
code. 
...
between a stack and a queue is that stack is based on Last in First out (LIFO) principle whilea 
...
type must be exactly same as is that of the method being overridden. Hence usinga different 
...
to call the non-Static methods from a static block (like froma static main method), an object of the 
...
Ans: In Java, if a variable is used ina code without prior initialization by a valid value, program 
...
Q67.What will be the output of Round(3.7) and Ceill3.7)? 
...
Q81. In Java thread programming, which method is a must implemetationfor all threads? 
...
Q84. I want my class to be developed in sucha way that no other class (even derived class) can 
...
Ans: Java provides a Collection APl which provides many useful methods which can be applied on a 
set of objects. Some of the important classes provided by Collection API include ArrayList,
Hash Map, TreeSet and TreeMap. 
...
Ans: In case of inheritance, whena new object of a derived class is created, first the constructor of 
